...
const TensBlock = ({ layoutId, isFaded = false }: { layoutId: string, isFaded?: boolean }) => (
    <motion.div 
        layoutId={layoutId} 
        className={`w-4 h-12 bg-blue-500 rounded transition-opacity ${isFaded ? 'opacity-20' : ''}`}
        initial={{ opacity: 0, y: -10 }}
        animate={{ opacity: 1, y: 0 }}
        exit={{ opacity: 0, scale: 0.5 }}
    />
);
const OnesBlock = ({ layoutId, isFaded = false }: { layoutId: string, isFaded?: boolean }) => (
    <motion.div 
        layoutId={layoutId} 
        className={`w-4 h-4 bg-black dark:bg-white rounded transition-opacity ${isFaded ? 'opacity-20' : ''}`} 
                                ...
                                <AnimatePresence>
                                    {Array.from({length: isSolved ? partsA.ones + 10 : partsA.ones}).map((_,i) => (
                                         <motion.div 
                                            ...
                                            initial={{ opacity: 0, scale: 0 }}
                                            animate={{ 
                                                opacity: isSolved && i >= (partsA.ones + 10 - partsB.ones) ? 0.2 : 1,
                                                scale: 1,
                                            }}
